...
//        }
//
//        // 付款失败：order设为失效 + 库存加回去 + redis库存加回去 + 限购锁解开（如果有）
//        if (order.getOrderStatus() != 2)  {
//            log.info("Didn't pay the order on time, order number：" + order.getOrderNo());
//
//            order.setOrderStatus(99);
//            onlineShoppingOrderDao.updateOrder(order);          //update order
//
//            onlineShoppingCommodityDao.revertStockWithCommodityId(order.getCommodityId());      //revert DB
//
//            String redisKey = "commodity:"  + order.getCommodityId();          //revert Redis
